...
static uma_zone_t avtab_node_cachep;
		  ...
		  struct avtab_key *key, struct avtab_datum *datum)
{
	...
	newnode = uma_zalloc(avtab_node_cachep, M_NOWAIT | M_ZERO);
	if (newnode == NULL)
		return NULL;
	newnode->key = *key;
	newnode->datum = *datum;
...
void avtab_destroy(struct avtab *h)
{
	...
	for (i = 0; i < AVTAB_SIZE; i++) {
		...
		while (cur != NULL) {
			temp = cur;
			cur = cur->next;
			uma_zfree(avtab_node_cachep, temp);
		}
		h->htable[i] = NULL;
	}
	kfree(h->htable);
...
{
	avtab_node_cachep = uma_zcreate("avtab_node", sizeof(struct avtab_node),
					NULL, NULL, NULL, NULL, UMA_ALIGN_PTR, 0);
}

void avtab_cache_destroy(void)
{
	uma_zdestroy(avtab_node_cachep);
}
