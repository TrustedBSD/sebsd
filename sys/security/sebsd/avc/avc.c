...
#define TE_(s) };
#define S_(s) s,
#include <security/sebsd/avc/common_perm_to_string.h>
#undef TB_
#undef TE_
#undef S_
...
#define avc_cache_stats_incr(field)	atomic_inc(&avc_cache_stats.field)
...
{
	uma_zfree(avc_node_cachep, node);
...
static struct avc_node *avc_alloc_node(void)
{
	...
	node = uma_zalloc(avc_node_cachep, M_NOWAIT | M_ZERO);
	if (!node)
		goto out;
