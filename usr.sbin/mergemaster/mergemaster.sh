...
# $FreeBSD: src/usr.sbin/mergemaster/mergemaster.sh,v 1.53 2006/02/04 18:24:06 rwatson Exp $
  ...
  *)
  ...
  '')
    { cd ${SOURCEDIR} &&
      ...
      '') ;;
      *)
      make DESTDIR=${DESTDIR} REALDESTDIR=${REALDESTDIR} ${ARCHSTRING} distrib-dirs
        ;;
      esac
      make DESTDIR=${TEMPROOT} REALDESTDIR=${REALDESTDIR} ${ARCHSTRING} distrib-dirs &&
	  ...
	  REALDESTDIR=${REALDESTDIR} DESTDIR=${TEMPROOT} distribution;} ||
    { echo '';
